...
class HnuterController:
    def __init__(self, model_path: str = "scene.xml"):
        ...
        self.KR = np.diag([0.5, 0.5, 0.5])   # 姿态增益
        self.Domega = np.diag([0.1, 0.1, 0.1])  # 角速度阻尼
    ...
    def get_state(self) -> dict:
        ...
        # print(f"四元数姿态: ({quaternion[0]:.2f}, {quaternion[1]:.2f}, {quaternion[2]:.2f}, {quaternion[3]:.2f})")
        # 转换为旋转矩阵
    ...
    def takeoff(self, target_height: float = 2.0):
        ...
        self.set_target_position(0.0, 1.0, target_height)
    ...
    def hover(self):
        ...
        self.set_target_position(0.0, 0.0, 2.0)
    ...
    def compute_control_wrench(self, state: dict) -> Tuple[np.ndarray, np.ndarray]:
        ...
        #     self.J @ ( np.cross(omega, R.T @ R_d @ self.target_attitude_rate) )  )
                      # - R.T @ R_d @ self.target_attitude_accel)
    ...
    def print_status(self):
        ...
        print(f"位置: ({pos[0]:6.2f}, {pos[1]:6.2f}, {pos[2]:6.2f}) \n "
              f"期望位置: ({self.target_position[0]:6.2f}, {self.target_position[1]:6.2f}, {self.target_position[2]:6.2f}) \n "
              f"惯性系控制力：{self.f_c_world[0]:5.2f}N {self.f_c_world[1]:5.2f}N {self.f_c_world[2]:5.2f}N \n "
              ...
              f"控制输入u: {self.u[0]:5.2f} {self.u[1]:5.2f} {self.u[2]:5.2f} {self.u[3]:5.2f} {self.u[4]:5.2f} \n "
    ...
    with viewer.launch_passive(controller.model, controller.data) as v:
        ...
        try:
            while v.is_running:
                ...
                controller.move_to(0.0, 0.0, 2.0)
                # 更新控制
                ...
                # 打印状态 (每0.1秒)
                if int(current_time * 10) % 10 == 0:  # 每0.5秒打印一次
