...
class HnuterController:
    def __init__(self, model_path: str = "scene.xml"):
        ...
        self.gravity = 9.81
        self.mass = 1.7 + 0.3 + 0.36  # 主机身质量 + 旋翼机构质量
        ...
        # 控制器增益 (根据论文设置)
        self.Kp = np.diag([50, 50, 50])  # 位置增益
        self.Dp = np.diag([1.5, 1.5, 2.0])  # 速度阻尼
        self.KR = np.diag([0.2, 0.3, 0.1])   # 姿态增益
        self.Domega = np.diag([0.5, 0.5, 0.2])  # 角速度阻尼
        
        # 几何控制器参数 (根据论文设置)
        self.geometric_params = {
            'position': {'kp': 10.0, 'kv': 15, 'ki': 0.2},
            'attitude': {'kr': 5.0 * np.array([1.5, 1.0, 1.0]),  # roll增益增加50%
                        'kw': 3.5 * np.array([1.8, 1.0, 1.0])}, # roll阻尼增加80%
        }
    ...
    def geometric_attitude_control(self, state: dict) -> np.ndarray:
        ...
        # 计算姿态误差矩阵
        R_error = target_rot.T @ R - R.T @ target_rot
        e_R_vec = np.array([R_error[2, 1], R_error[0, 2], R_error[1, 0]])  # vee操作
        ...
        # 力矩计算公式（包含科里奥利项补偿）
        torque = -kr * e_R_vec - kw * e_omega + np.cross(omega, self.J @ omega)
    ...
    def compute_control_wrench(self, state: dict) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # f_c_world = self.mass * (self.Kp @ e_p + self.Dp @ e_v + np.array([0, 0, 0]))
        f_c_body = R.T @ f_c_world
        # f_c_body[2] = max(f_c_body[2], 0.0)  # 关键限幅操作
        # 姿态控制律
    ...
    def allocate_actuators(self, f_c_body: np.ndarray, tau_c: np.ndarray):
        ...
        # 计算推力（论文公式9）
        # 修正后的推力计算（保留符号）
        T12 = -1 * np.sign(u[1]) * np.sqrt(u[0]**2 + u[1]**2)  # 前左组推力，符号由u[1]决定
        print(np.sign(u[1]))
        T34 = -1 * np.sign(u[3]) * np.sqrt(u[2]**2 + u[3]**2)  # 前右组推力，符号由u[3]决定
        T5 = u[4]                        # 尾部推进器推力
...
def main():
    ...
    # 设置飞行任务：起飞→悬停→降落
    flight_phases = [
        ("起飞", 0.0, [0,0,1.5], None, [0,0,0]),
        ("悬停", 5.0, [0,0,1.5], None, [0,0,0]),
        ("降落", 10.0, [0,0,0.3], None, [0,0,0])
    ]
    phase_index = 0
    ...
    with viewer.launch_passive(controller.model, controller.data) as v:
        ...
        try:
            while v.is_running:
                ...
                # 飞行阶段管理
                # if phase_index < len(flight_phases):
                #     phase_name, phase_delay, target_pos, target_att = flight_phases[phase_index]
                #     if current_time >= phase_delay:
                #         print(f"\n切换到阶段: {phase_name} | 目标高度={target_pos[2]}m")
                #         controller.set_target_position(*target_pos)
                #         controller.set_target_attitude(*target_att)
                #         phase_start_time = current_time
                #         phase_index += 1
                
                controller.set_target_position(0,0,1.5)
