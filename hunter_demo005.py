...
class HnuterController:
    ...
    #     # 计算期望的z轴方向
    #     if np.linalg.norm(f_c_world) < 1e-6:
    #         # 当控制力很小时，使用默认方向
    #         z_d = np.array([0, 0, 1])
    #     else:
    #         z_d = f_c_world / np.linalg.norm(f_c_world)
        
    #     # 获取参考的x轴（由目标偏航角决定）
    #     yaw = self.target_attitude[2]
    #     x_ref = np.array([np.cos(yaw), np.sin(yaw), 0])
    ...
    #     # 构造目标旋转矩阵
    #     R_WB_d = np.column_stack([x_d, y_d, z_d])
    #     # print(R_WB_d)
        
    #     # 正交化处理（防止数值误差）
    #     U, _, Vt = np.linalg.svd(R_WB_d)
    #     return U @ Vt
    ...
    # def compute_target_rotation(self, f_c_body: np.ndarray) -> np.ndarray:
    #     """计算目标旋转矩阵（修正不可控自由度）"""
    #     # 步骤1：构建修正控制力向量（忽略机体无法产生的Y轴力）
    #     f_c_body_modified = np.array([0, f_c_body[1], f_c_body[2]])  # 保留X、Z分量，Y分量置零
        
    #     # 步骤2：计算世界坐标系下的总推力方向（Z轴）
    #     R_WB = self.get_state()['rotation_matrix']
    #     f_c_world_modified = R_WB @ f_c_body_modified
        
    #     f_norm = np.linalg.norm(f_c_world_modified)
    #     if f_norm < 1e-6:
    #         z_d = np.array([0, 0, 1])  # 零推力时默认垂直向上
    #     else:
    #         z_d = f_c_world_modified / f_norm  # 归一化获得Z轴方向
        
    #     # 步骤3：构造参考X轴（融合目标俯仰角与偏航角）
    #     roll = self.target_attitude[0]
    #     pitch = self.target_attitude[1]  # 目标俯仰角
    #     yaw = self.target_attitude[2]    # 目标偏航角
    ...
    #         [0, 0, 1]
    #     ])
    #     x_ref =  R_roll @ R_pitch @ R_yaw @ np.array([1, 0, 0])  # 世界系下的参考X轴
        
    #     # 步骤4：构建正交坐标系（关键修正）
    #     # 计算Y轴：垂直于推力方向与参考X轴构成的平面
    #     y_d = np.cross(z_d, x_ref)
    #     y_norm = np.linalg.norm(y_d)
    #     if y_norm < 1e-6:
    #         # 退化处理：当z_d与x_ref平行时使用备用Y轴
    #         y_d = np.array([0, 1, 0]) if abs(z_d[0]) < 0.8 else np.array([1, 0, 0])
    #     else:
    #         y_d /= y_norm
        
    #     z_d = np.cross(x_ref,y_d)
    #     z_norm = np.linalg.norm(z_d)
    #     if z_norm < 1e-6:
    #         # 退化处理
    #         z_d = np.array([0, 0, 1])
    #     else:
    #         z_d /= z_norm
    #     # 调试信息（验证坐标系正交性）
    #     # print(f"正交性验证: X·Y={x_d.dot(y_d):.2e}, Y·Z={y_d.dot(z_d):.2e}, Z·X={z_d.dot(x_d):.2e}")
        
    #     return np.column_stack([x_ref, y_d, z_d])  # 目标旋转矩阵 [X_d, Y_d, Z_d]
    ...
    def calculate_smooth_angle(self, x, y, last_angle):
        """计算平滑过渡的倾转角（避免±180°跳变）"""
        current_angle = np.arctan2(abs(x), abs(y))
        # 角度解缠绕处理
...
def mobius_trajectory(t, period=20.0, radius=1.0, height=2.5):
    """生成莫比乌斯环轨迹参数[7,8](@ref)"""
    u = 2 * np.pi * t / period
    ...
    with viewer.launch_passive(controller.model, controller.data) as v:
        ...
        try:
            while v.is_running:
                ...
                target_pos, target_vel, target_yaw = mobius_trajectory(current_time)
            
                # 设置目标状态
                controller.set_target_position(*target_pos)
                controller.set_target_velocity(*target_vel)
                controller.set_target_attitude(0, 0, target_yaw)
                # 获取当前状态
