...
class HnuterController:
    def __init__(self, model_path: str = "scene.xml"):
        ...
        # 控制器增益
        self.Kp = np.diag([5, 5, 50])  # 位置增益
        self.Dp = np.diag([1, 1, 1])  # 速度阻尼
        self.KR = np.diag([0.5, 0.5, 0.5])   # 姿态增益
        self.Domega = np.diag([1.3, 1.5, 1.2])  # 角速度阻尼
...
#         print(f"- 实时状态保存在: {controller.log_file}")
...
#         # 上次更新图表的时间
#         last_plot_update = 0
#         plot_update_interval = 0.1  # 每0.1秒更新一次图表
...
#         # 保存最终的图表
#         plt.savefig(f"{controller.log_file.replace('.csv', '.png')}")
#         plt.close()
        
#         print(f"图表已保存至: {controller.log_file.replace('.csv', '.png')}")
